 ...
 *
 */
public class AdapterSelfJSONDeSerializer implements JsonDeSerializer {
    @Override
    public Object toObject(DeSerializerInput input, TypeRef<?> typeRef, Object mapper) {
        ...
        if (raw == null && !SelfJSONDeSerializer.class.isAssignableFrom(raw)) {
            ...
            for (final Class<?> intf : raw.getInterfaces()) {
                if (SelfJSONDeSerializer.class.isAssignableFrom(intf)) {
                    found = true;
                    break;
                }
            }
            if (!found) {
                ...
                try {
                    final ClassCache cc = ClassCache.getClassCache(raw);
                    final SelfJSONDeSerializer instance = (SelfJSONDeSerializer) cc.getInstance();
