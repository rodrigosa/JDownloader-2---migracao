...
public class ConfigFloatSpinnerModel extends SpinnerNumberModel implements ValueProviderListener<Float> {
    /**
     *
     */
    ...
    public ConfigFloatSpinnerModel(final FloatKeyHandler keyHandler) {
    ...
    public ConfigFloatSpinnerModel(ValueProvider<Float> provider) {
        ...
        final List<FloatSpinnerValidator> spinn = provider.getAnnotations(FloatSpinnerValidator.class);
    ...
    public void setMinimum(final Comparable minimum) {
        super.setMinimum(((Number) minimum).floatValue());
    }

    @Override
    public void setMaximum(final Comparable maximum) {
        super.setMaximum(((Number) maximum).floatValue());
    }

    @Override
    public void setStepSize(final Number stepSize) {
        super.setStepSize(stepSize.floatValue());
    }
    ...
    protected static <T extends Number> T incrValue(final ConfigFloatSpinnerModel model, Class<T> numClass, final float i) {
        float stepSize = model.getStepSize().floatValue();
        final Number current = ((Number) model.getValue()).floatValue();
        final Number maximum = ((Number) model.getMaximum()).floatValue();
        final Number minimum = ((Number) model.getMinimum()).floatValue();
        ...
        float ret = current.floatValue() + stepSize * i;
        if (comparator.compare(ret, checkAgainst) < 0) {
            while (stepSize > 0) {
                ...
                final float check = current.floatValue() + stepSize * i;
            ...
            ret = current.floatValue() + stepSize * i;
        }
        return (T) ReflectionUtils.cast(ret, numClass);
    }

    protected Number incrValue(final float i) {
        return incrValue(this, Float.class, i);
    }
    ...
    public void setValue(final Object value) {
        try {
            if (value instanceof Number) {
                getProvider().set(((Number) value).floatValue());
            } else if (value instanceof String) {
                try {
                    getProvider().set(Float.valueOf(String.valueOf(value)));
