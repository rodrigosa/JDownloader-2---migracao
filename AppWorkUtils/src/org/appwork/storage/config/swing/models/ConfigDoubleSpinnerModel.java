...
public class ConfigDoubleSpinnerModel extends SpinnerNumberModel implements ValueProviderListener<Double> {
    /**
     *
     */
    ...
    public ConfigDoubleSpinnerModel(final DoubleKeyHandler keyHandler) {
    ...
    public ConfigDoubleSpinnerModel(ValueProvider<Double> provider) {
        ...
        final List<DoubleSpinnerValidator> spinn = provider.getAnnotations(DoubleSpinnerValidator.class);
    ...
    protected static <T extends Number> T incrValue(final ConfigDoubleSpinnerModel model, Class<T> numClass, final double i) {
        double stepSize = model.getStepSize().doubleValue();
        final Number current = ((Number) model.getValue()).doubleValue();
        final Number maximum = ((Number) model.getMaximum()).doubleValue();
        final Number minimum = ((Number) model.getMinimum()).doubleValue();
        ...
        double ret = current.doubleValue() + stepSize * i;
        if (comparator.compare(ret, checkAgainst) < 0) {
            while (stepSize > 0) {
                ...
                final double check = current.doubleValue() + stepSize * i;
            ...
            ret = current.doubleValue() + stepSize * i;
        }
        return (T) ReflectionUtils.cast(ret, numClass);
    }

    protected Number incrValue(final double i) {
        return incrValue(this, Double.class, i);
    }
    ...
    public void setValue(final Object value) {
        try {
            if (value instanceof Number) {
                getProvider().set(((Number) value).doubleValue());
            } else if (value instanceof String) {
                try {
                    getProvider().set(Double.valueOf(String.valueOf(value)));
