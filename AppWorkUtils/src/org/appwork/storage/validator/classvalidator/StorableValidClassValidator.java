 ...
 *
 */
public class StorableValidClassValidator extends StorableAbstractValidator {
    public static class InvalidClassStringException extends ValidatorException {
        /**
         * @param validator
         * @param cause
         * @param path
         ...
         * @param failLevel
         */
        public InvalidClassStringException(StorableValidator validator, Throwable cause, JSPath path, FlexiJSonNode value, CompiledType targetType, String message, FailLevel failLevel) {
    ...
    public List<? extends ValidatorException> validate(StorableValidator validator, Object root, Object value, FlexiJSonNode node, JSPath path, CompiledType type, String parameter, FailLevel level, String message) {
        ArrayList<InvalidClassStringException> ret = new ArrayList<InvalidClassStringException>();
        if (!(value instanceof String)) {
            ret.add(new InvalidClassStringException(validator, null, path, node, type, StringUtils.isEmpty(message) ? ("The value must be a fully qualified Classname String") : message, level));
        }
        try {
            Class.forName((String) value, false, getClass().getClassLoader());
        } catch (ClassNotFoundException e) {
            ret.add(new InvalidClassStringException(validator, e, path, node, type, StringUtils.isEmpty(message) ? ("Failed to load Class.The value must be a fully qualified Classname String") : message, level));
        }
        return ret;
    }
    ...
    public String getDocsDescription(String parameter, Object anno) {
        return "The value must be a fully qualified Classname String";
    }
}
