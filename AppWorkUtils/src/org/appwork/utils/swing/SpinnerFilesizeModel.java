...
public class SpinnerFilesizeModel extends SpinnerNumberModel {
    ...
    public Object getNextValue() {
        try {
            ...
            final Integer ret = ((Number) super.getNextValue()).intValue() / step;
            return ret * step;
        } catch (Exception e) {
            return null;
        }
    ...
    private int getStep() {
        final Number value = (Number) this.getValue();
        int step;
        if (value.intValue() < 10 * 1000) {
            step = 1000;
        } else {
            final int base = (int) Math.pow(1000, (int) (Math.log(value.intValue()) / Math.log(1000)));
    ...
    public Object getPreviousValue() {
        try {
            final int step = getStep();
            final Integer ret = ((Number) super.getPreviousValue()).intValue() / step;
            return ret * step;
        } catch (Exception e) {
            return null;
        }
