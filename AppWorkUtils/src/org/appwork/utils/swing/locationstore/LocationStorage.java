...
public class LocationStorage implements Storable {
    ...
    /**
     * returns true if anything has changed, and false in case of no changes
     *
     * @param type
     *            TODO
     * @param b
     ...
     * @return
     */
    public boolean update(final boolean valid, final String type, int x, int y) {
        if (isValid() == valid && getX() == x && getY() == y && StringUtils.equals(type, getType())) {
            return false;
        } else {
            setType(type);
            setValid(valid);
