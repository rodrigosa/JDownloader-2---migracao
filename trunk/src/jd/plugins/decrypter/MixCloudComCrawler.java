...
@DecrypterPlugin(revision = "$Revision: 49846 $", interfaceVersion = 3, names = { "mixcloud.com" }, urls = { "https?://(?:www\\.)?mixcloud\\.com/(widget/iframe/\\?.+|[^/]+/?([^/]+/)?)" })
public class MixCloudComCrawler extends antiDDoSForDecrypt {
    ...
    private ArrayList<DownloadLink> crawlUsername(final CryptedLink param) throws Exception {
        ...
        final String csrftoken = findCsrftoken(br);
        if (csrftoken == null) {
            throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
        }
        ...
        request = br.createJSonPostRequest("https://app.mixcloud.com/graphql", "{\"query\":\"" + query + "\",\"variables\":{\"lookup\":{\"username\":\"" + username + "\"},\"orderBy\":\"LATEST\"}}");
        br.getPage(request);
        ...
        do {
            ...
            request = br.createJSonPostRequest("https://app.mixcloud.com/graphql", "{\"query\":\"" + queryPagination + "\",\"variables\":{\"count\":20,\"cursor\":\"" + lastCursor + "\",\"orderBy\":\"LATEST\",\"userID\":\"" + userIDb64 + "\"}}");
            br.getPage(request);
            page++;
        } while (!this.isAbort());
        return ret;
    ...
    private ArrayList<DownloadLink> crawlSingleAudio(final CryptedLink param, final String parameter) throws Exception {
        ...
        int totalNumberofDetectedAudioObjects = 0;
        do {
            ...
            if (page == 0) {
                ...
                String xcsrftoken = br.getCookie(br.getHost(), "csrftoken", Cookies.NOTDELETEDPATTERN);
            ...
            processAudioObjects(ret, audioObjects, fp, dupes);
            totalNumberofDetectedAudioObjects += audioObjects.size();
            page++;
        } while (hasMore);
        /* Add thumbnail if possible. */
        if (!StringUtils.isEmpty(url_thumbnail)) {
            final DownloadLink dlink = createDownloadlink(url_thumbnail);
            dlink.setFinalFileName(title + ".jpeg");
            dlink._setFilePackage(fp);
            ret.add(dlink);
        }
        if ((ret == null || ret.size() == 0) && totalNumberofDetectedAudioObjects > 0) {
            /* E.g. all streams display error "Sorry, this stream is currently unavailable" */
            logger.info("Found audio streams but all of them were offline | Number of streams: " + totalNumberofDetectedAudioObjects);
            throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
    ...
    public static String findCsrftoken(final Browser br) throws PluginException, IOException {
        ...
        if (csrftoken == null) {
            ...
            prepBrAjax(brc, null);
            final PostRequest request = brc.createJSonPostRequest("https://app.mixcloud.com/graphql", "{\"query\":\"query GoogleAdSdkQuery {  viewer {    ads {      targeting {        key        value      }    }    id  }}\",\"variables\":{}}");
            brc.getPage(request);
            csrftoken = brc.getCookie(brc.getHost(), "csrftoken", Cookies.NOTDELETEDPATTERN);
            if (csrftoken == null) {
                throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
            }
            prepBrAjax(br, csrftoken);
        }
        return csrftoken;
    }

    private void processAudioObjects(final ArrayList<DownloadLink> decryptedLinks, final List<Object> audio_objects, final FilePackage fp, final ArrayList<String> dupes) {
