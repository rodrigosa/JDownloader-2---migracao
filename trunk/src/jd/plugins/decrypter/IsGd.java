...
public class IsGd extends PluginForDecrypt {
    ...
    public ArrayList<DownloadLink> decryptIt(final CryptedLink param, ProgressController progress) throws Exception {
        ...
        if (finallink == null || finallink.contains("is.gd/")) {
            finallink = br.getRegex("(?i)the destination shown: \\-<br /><a href=\"(http[^<>\"]*?)\"").getMatch(0);
        }
        if (br.getHttpConnection().getResponseCode() == 404) {
            throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
        } else if (br.containsHTML(">\\s*Sorry, we couldn't find the shortened URL you requested") || br.containsHTML(">\\s*Link Disabled\\s*<")) {
            throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
        }
