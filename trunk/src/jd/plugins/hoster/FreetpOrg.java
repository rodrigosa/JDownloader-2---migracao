...
public class FreetpOrg extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        String filename = br.getRegex("Имя:</b></font>([^<>\"]+)<br").getMatch(0);
        String filesize = br.getRegex("Размер:</b></font>([^<>\"]+)<").getMatch(0);
        filename = Encoding.htmlDecode(filename).trim();
        if (StringUtils.isEmpty(filename)) {
            filename = getLinkID(link);
            /* Do not set final filename here as we failed to find it */
            link.setName(filename);
        } else {
            /* Filehost tags filenames so let's set final filename here */
    ...
    private void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink == null) {
            dllink = String.format("http://%s/engine/download.php?id=%s&area=", br.getHost(), this.getLinkID(downloadLink));
