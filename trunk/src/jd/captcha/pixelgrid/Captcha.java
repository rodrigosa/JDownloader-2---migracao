...
public class Captcha extends PixelGrid {
            ...
            for (int y = 0; y < getHeight(); y++) {
                ...
                if (Colors.getColorDifference(mask.getPixelValue(x, y), getPixelValue(x, y)) < dif) {
                    ...
                    for (int i = 1; i < tolerance; i++) {
                        if (y + i < getHeight()) {
                            if (Colors.getColorDifference(mask.getPixelValue(x, y + i), getPixelValue(x, y + i)) < dif) {
                                del += tolerance - i;
                            }
                            del2 += tolerance - i;
                        } else {
                    ...
                    for (int i = 1; i < tolerance; i++) {
                        if (x + i < getWidth()) {
                            if (Colors.getColorDifference(mask.getPixelValue(x + i, y), getPixelValue(x + i, y)) < dif) {
                                del += tolerance - i;
                            }
                            del2 += tolerance - i;
                        } else {
    ...
    public Letter[] getLetters0(int letterNum) throws InterruptedException {
        ...
        if (owner.getJas().getBoolean("cancelIfObjectDetectionFailed")) {
            return null;
        }
    ...
    private void mergeMultiPartObjects(Vector<PixelObject> objects) {
        if (owner.getJas().getInteger("multiplePartMergeMinSize") <= 0) {
            return;
        }
