    ...
    protected void onActionPerformed(ActionEvent e, List<Archive> archives, SelectionInfo<?, ?> selectionInfo) {
        try {
            ...
            if (archives.size() > 1) {
                ...
                pw = Dialog.getInstance().showInputDialog(0, org.jdownloader.extensions.extraction.translate.T.T.context_password(), (list == null || list.size() == 0) ? org.jdownloader.extensions.extraction.translate.T.T.context_password_msg(archives.get(0).getName()) : org.jdownloader.extensions.extraction.translate.T.T.context_password_msg2(archives.get(0).getName(), sb.toString()), null, NewTheme.getInstance().getIcon("password", 32), null, null);
            }
